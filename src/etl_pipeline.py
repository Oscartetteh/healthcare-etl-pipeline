...
from config import (
    ...
    REPORT_DOCX_PATH,   # where to save the Word report
    REPORT_XLSX_PATH,   # where to save the Excel report
...
from src.report_writer import export_report_docx, export_report_xlsx
...
class ETLPipeline:
    ...
    def extract(self) -> "ETLPipeline":
        ...
        into a usable format. Module already extracted it FROM the database.
    ...
    def report(self) -> None:
        ...
        docx_path = export_report_docx(self, transform_summary, REPORT_DOCX_PATH)
        xlsx_path = export_report_xlsx(self, transform_summary, REPORT_XLSX_PATH)
        self._log(f"Word report saved to {docx_path}")
        self._log(f"Excel report saved to {xlsx_path}")
        print()
